...
אוהד הוא מנהל מוצר (Product Manager) ובעל תואר שני בהצטיינות (M.A.) בניהול ארגוני שירות.
רקע מקצועי של אוהד:
- ניסיון רב בניהול מוצר והובלת מערכות SaaS מורכבות בסביבות B2B מקומיות ובארגוני Enterprise גלובליים.
- מומחיות עמוקה בעולמות הפינטק, ההתחשבנות והשילוח הבינלאומי, לצד רקע עשיר בעולם הקמעונאות והשירות.
- אפיון דרישות ומסמכי PRD, מיפוי פרסונות ומסעות משתמש, ואפיון ממשקי APIs וזרימת נתונים.
- קבלת החלטות מבוססות דאטה בעזרת שאילתות SQL ישירות, והובלת פרויקטי הגירת נתונים מ-On-Premise לענן בריבוי חברות תוך בקרת איכות ושלמות מידע.
- התאמת מוצרים ותהליכים לרגולציה, אבטחת מידע ותקנים בישראל.
- אפיון נהלי עבודה סדורים (SOPs) לכל רוחב העסק כבסיס להטמעת סוכני AI ממוקדים עם מנגנוני בקרה (Guardrails) ומדידת ROI (פרויקט BinaTor משמש כ-Case Study מעשי בלייב).

אוהד מעניק שירות פרימיום מותאם אישית (Boutique) לעסקים והנהלות:
1. אפיון וכתיבת נהלי עבודה (SOPs) לכל רוחב העסק — בדגש על שימור ידע, המשכיות עסקית, סקיילביליות (Scalability) וערך ממשי שאינו תלוי בעובד בודד.
2. סדנאות Hands-on מעשיות להכשרת צוותים והנהלה, כולל הנדסת פרומפטים ובניית סוכני AI ממוקדים.
3. אבחון תהליכים, מיפוי צווארי בקבוק ומדידת חיסכון בשעות עבודה (ROI).
4. פיתוח סוכני AI ואוטומציות קצה-לקצה מותאמות אישית.
