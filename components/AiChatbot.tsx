  ...
  return (
    <>
      ...
      >
        ...
        >
          <div className="relative flex items-center justify-center text-xl">
            <span>{isOpen ? '✕' : '🤖'}</span>
            {!isOpen && (
              <span className="absolute -top-1 -right-1 w-3 h-3 bg-emerald-400 rounded-full border-2 border-[#070A10] animate-pulse" />
            )}
          </div>
      ...
      {isOpen && (
        ...
        >
          <div className="bg-[#070A10] px-5 py-4 border-b border-slate-800 flex items-center justify-between shrink-0">
          ...
          <div
            ref={listRef}
            className="flex-1 overflow-y-auto px-4 py-4 space-y-3 bg-[#070A10] ai-chat-stream"
            dir="rtl"
          >
            {messages.map((m) => (
              <div
                ...
                <div
                  className={`max-w-[88%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed whitespace-pre-wrap ai-chat-bubble ${
                    m.role === 'user'
                      ...
                      : 'bg-slate-800 text-slate-100 border border-slate-700 rounded-br-md'
                  }`}
                  dir="rtl"
                >
          ...
          {error && (
            <div className="px-4 py-2 text-xs text-amber-300 bg-amber-950/40 border-t border-amber-800/50 shrink-0" dir="rtl">
              {error}
            </div>
          )}

          <form
            className="p-3 border-t border-slate-800 bg-[#0D131F] flex gap-2 shrink-0"
            dir="rtl"
            onSubmit={(e) => {
              e.preventDefault();
              void sendMessage();
          ...
          >
            <input
              ...
              className="flex-1 rounded-xl bg-slate-900 border border-slate-700 text-white text-sm px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 disabled:opacity-60"
              dir="rtl"
